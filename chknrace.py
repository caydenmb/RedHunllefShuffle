...
def fetch_data():
    ...
    try:
        ...
        if response.status_code == 400 and "INVALID_DATE" in response.text:
            ...
            log_message('debug', f"Fallback status code: {response.status_code}")

        if response.status_code == 200:
            api_response = response.json()
            log_message('info', f"API returned {len(api_response) if isinstance(api_response, list) else 'N/A'} entries")
            ...
            log_message('error', f"Fetch failed: {response.status_code}")
...
def schedule_data_fetch():
    log_message('info', 'Scheduling next data fetch in 90 seconds')
    fetch_data()
