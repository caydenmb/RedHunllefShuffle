/* Minimal, dependency-free stats rendering (SVG). */
...
(function(){
  ...
  function drawBars(svg, data){
    ...
    data.forEach((row,i)=>{
      ...
      rect.setAttribute('width', Math.max(2,wid)); rect.setAttribute('height', barH-12);
      rect.setAttribute('fill', '#2f7ad1'); svg.appendChild(rect);

      const label=document.createElementNS('http://www.w3.org/2000/svg','text');
      label.setAttribute('x', pad+6); label.setAttribute('y', y + (barH/2));
      label.setAttribute('dominant-baseline','middle'); label.setAttribute('fill','#eaeaea'); label.setAttribute('font-size','12');
      label.textContent = `${row.referrer||'—'} • ${fmt(row.count||0)}`; svg.appendChild(label);
    });
  }

  function drawDonut(svg, legendEl, map){
    svg.innerHTML=''; legendEl.innerHTML='';
    const entries = Object.entries(map||{}); const total = entries.reduce((a,[,v])=>a+(v||0),0);
    if(entries.length===0 || total===0){ svg.textContent='No data'; return; }
    const cx=300, cy=110, r=70, thick=26; const COLORS={mobile:'#3abf5a',desktop:'#8884ff',tablet:'#ff9f43'};
    let start=0;
    entries.forEach(([k,v])=>{
      const frac=(v||0)/total, end=start+frac*2*Math.PI;
      ...
      const d = [
        ...
        `A ${r-thick} ${r-thick} 0 ${large} 0 ${cx + Math.cos(start)*(r-thick)} ${cy + Math.sin(start)*(r-thick)}`, 'Z'
      ].join(' ');
      path.setAttribute('d', d); path.setAttribute('fill', COLORS[k] || '#666'); svg.appendChild(path);

      const leg=document.createElement('div');
      leg.textContent = `${k} — ${fmt(v||0)} (${pct(v,total)}%)`; legendEl.appendChild(leg);
      start=end;
    });
    const title=document.createElementNS('http://www.w3.org/2000/svg','text');
    title.setAttribute('x', 300); title.setAttribute('y', 110);
    title.setAttribute('text-anchor','middle'); title.setAttribute('fill','#ddd'); title.setAttribute('font-weight','900');
    title.textContent = `${fmt(total)} visits`; svg.appendChild(title);
  }
  ...
  function drawTimeline(svg, timeline){
    ...
    const bandY = h/2 - 12, bandH = 24; const liveColor='#ff5c5c', offColor='#2a2a2a';
