/* Minimal, dependency-free stats rendering.
   Draws simple SVG charts from /stats-data. */
...
(function(){
  ...
  function drawArea(svg, data){
    ...
    data.forEach((pt,i)=>{
      const x=pad + i*step;
      const y=h-pad - (pt.count/max)*(h-2*pad);
      d+=` L ${x} ${y}`;
    });
  ...
  function drawBars(svg, data){
    ...
    data.forEach((row,i)=>{
      ...
      const wid=(row.count/max)*(w-2*pad);
      ...
      label.textContent = `${row.referrer} • ${fmt(row.count)}`;
  ...
  function drawDonut(svg, legendEl, map){
    ...
    Object.entries(map).forEach(([k,v])=>{
      ...
      leg.textContent = `${k} — ${fmt(v)} (${pct(v,total)}%)`;
    ...
    title.textContent = total ? `${fmt(total)} visits` : 'No data';
